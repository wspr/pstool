...
%    \begin{macrocode}
\ProvidesPackage{pstool}[2013/02/05 v1.4a
...
%
% v1.4a: Use |\filemodCmp| instead of |\pstool@IfnewerEPS| -- Heinrich Kruger
...
% Emulate |\include| and have the main document load the auxiliary aux file, in a manner of speaking:
    \pstool@write@aux
  }%
...
% This is because the aux file is cleared for writing after |\begin{document}|.
...
% And the document body to place the graphic on a page of its own:
    \noexpand\makeatletter
    \noexpand\@input{\jobname.oldaux}%
    \noexpand\begin{document}^^J%
    \unexpanded{\immediate\write\@mainaux}{\pstool@auxmarker*}%
    \noexpand\makeatother
    \unexpanded{%
      \centering\null\vfill^^J%
    }%
    ...
    \fi
    \unexpanded{\makeatletter\immediate\write\@mainaux}{\pstool@auxmarker/}^^J%
    \unexpanded{\makeatother\vfill\end{document}}^^J%
  }%
...
\textbf{New in 1.4a}: recently, the behaviour of |ps2pdf| has changed under Windows.

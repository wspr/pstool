...
Running `latex` on pstool.tex will produce the files
  pstool.ins, pstool.sty, pstool.drv, and README.txt,
and compile the PDF documentation.

Executing:
  `tex pstool.ins` produces the files above except pstool.ins.
  `latex pstool.drv` produces the typeset documentation.
...
\ProvidesPackage{pstool}[2008/08/07 v0.4 
...
%
% These all support the suffixes \verb|*| and \verb|!|, so each user command is defined as a wrapper to \cmd\pstool.

...
\ExecuteOptionsX{cleanup={.tex,.dvi,.ps,.pdf,.log,.aux,-blx.bib,.nav,.out,.snm,.toc}}
...
\def\pstool@retrievestatus#1#2{%
  \CatchFileEdef{#2}{#1\pstool@statusfile}{}%
  \pstool@rm{\pstool@statusfile}%
  \ifx#2\pstool@statusfail
    \PackageWarning{pstool}{%
      Status of process unable to be determined:^^J  #1^^J%
      Trying to proceed... }%
    \def#2{0}%
  \fi}
\def\pstool@statusfail{\par }
...
\def\pstool@datefiles{%
  ...
  \pstool@retrievestatus{\ip@directpath}{\@tempa}%
...
\def\pstool@@psfragfig#1[#2]#3#4{%
  \IfFileExists{#3.eps}{%
    \def\pstool@eps{#3}%
    \OnlyIfFileExists{#3-psfrag.eps}{%
      \PackageWarning{pstool}{Graphic "#3-psfrag.eps" exists but "#3.eps" is being used}%
    }%
  }{%
    \IfFileExists{#3-psfrag.eps}{%
      \def\pstool@eps{#3-psfrag}%
    }{%
      \PackageError{pstool}{%
        No EPS file "#3.eps" or "#3-psfrag.eps" found%
      }{%
  ...
  #1[#2]{\pstool@eps}{%
    \InputIfFileExists{#3-psfrag.tex}{%
      \OnlyIfFileExists{#3.tex}{%
        \PackageWarning{pstool}{%
          File "#3.tex" exists that may contain macros for "#3.eps"^^J%
          But file "#3-psfrag.tex" is being used instead.}%
      }%
    }{%
      \InputIfFileExists{#3.tex}{}{}%
    }%
    #4}%
}
